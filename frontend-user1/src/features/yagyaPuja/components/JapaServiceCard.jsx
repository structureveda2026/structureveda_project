import { MapPin, Sparkles, ArrowRight, Clock, Users, Hash } from "lucide-react";
...
const JapaServiceCard = ({ service }) => {
  if (!service) return null;

  return (
    <Link
      to={`/yagya-puja/japa/${service.slug}`}
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2 text-[12px] font-semibold text-[#8a561c]">
            <div className="flex items-center gap-1">
              ...
              <span>{service.dailyHours || "4 Hours / Day"}</span>
            </div>
            <span>•</span>
            <span className="text-[#75695c]">{service.completionWindow || "Multi-Day"}</span>
          </div>
          ...
          {/* Mantra Excerpt Inset */}
          {service.mantra && (
            <div className="mt-2.5 rounded-xl border border-[#ebd6be] bg-[#fbf6ed] px-3.5 py-2">
              <p className="font-serif text-[12.5px] italic text-[#634928] line-clamp-1">
                {service.mantra}
              </p>
            </div>
          )}
          ...
          {/* Description */}
          <p className="mt-2.5 text-[13.5px] leading-relaxed text-[#685c4f] line-clamp-2">
            {service.shortDescription || service.description}
          </p>

          {/* Service-Specific Available Counts Pills */}
          <div className="mt-4">
            <span className="text-[10.5px] font-bold uppercase tracking-wider text-[#8a725b]">
              Available Counts:
            </span>
            <div className="mt-1.5 flex flex-wrap gap-1.5">
              {(service.availableCounts || []).map((cnt) => (
                <span
                  key={cnt}
                ...
                >
                  <Hash size={10} className="text-[#b36c1e]" />
                  {formatCount(cnt)}
                </span>
              ))}
            </div>
          ...
          {service.recommendedPandits && (
            <div className="mt-3.5 flex items-center gap-1.5 text-[12px] font-medium text-[#7d674b]">
              ...
              <span>Calculated team: {service.minimumPandits} – {service.recommendedPandits} Vedic Pandits</span>
            </div>
          )}
        </div>
...
export default JapaServiceCard;
