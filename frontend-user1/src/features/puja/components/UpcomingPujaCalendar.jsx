...
import PujaByPurpose from "./PujaByPurpose";
...
const UpcomingPujaCalendar = ({ selectedPurpose, onSelectPurpose }) => {
  ...
  return (
    ...
    >
      ...
      {/* ── Main Calendar Container (3-Column Layout on Desktop) ── */}
      <div className="rounded-[24px] border border-[#ebdcc4] bg-[#fbf6ec]/60 p-4 sm:p-5 lg:p-6 shadow-xs">
        <div className="grid gap-5 lg:grid-cols-12 lg:gap-5 xl:gap-6 items-stretch">
          
          {/* =========================================================
              PANEL 1: MONTH-WISE INTERACTIVE CALENDAR GRID (4 cols)
          ========================================================== */}
          <div className="flex flex-col justify-between rounded-[20px] border border-[#ebdcc4] bg-[#fffdfa] p-4 sm:p-5 shadow-2xs lg:col-span-4 h-full">
            <div>
              ...
              <div className="mb-4 flex items-center justify-between border-b border-[#f0e2cd] pb-3">
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded-[8px] border border-[#ebdcc4] bg-[#fffaf0] px-2.5 py-1.5 text-[11.5px] font-semibold text-[#423629] shadow-2xs transition-all duration-200 hover:border-[#c77722] hover:bg-[#fffdfa] hover:text-[#c77722] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#eab12c] cursor-pointer"
                >
                  <ChevronLeft size={14} />
                  <span>Prev</span>
                </button>
                ...
                <div className="text-center">
                  <span className="font-serif text-[17px] sm:text-[18px] font-bold tracking-wide text-[#2b241d]">
              ...
              <div className="grid grid-cols-7 text-center text-[11px] font-bold uppercase tracking-wider text-[#8a7a68] pb-2">
                {WEEKDAYS.map((day) => (
              ...
              <div className="grid grid-cols-7 gap-1 pt-1 sm:gap-1.5">
                {calendarGrid.map((cell) => {
                  if (!cell.isCurrentMonth) {
                    return (
                      <div
                        key={cell.key}
                        aria-hidden="true"
                        className="h-8 sm:h-9 rounded-[8px] opacity-10"
                      />
                    );
                  }
                  ...
                  return (
                    <button
                      ...
                      className={`relative flex h-8 sm:h-9 flex-col items-center justify-center rounded-[8px] text-[12px] sm:text-[12.5px] font-semibold transition-all duration-150 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#eab12c] cursor-pointer ${
                        isSelected
                          ? "bg-[#2b241d] text-[#f5ce6f] shadow-xs scale-[1.03]"
                          : hasEvents
            ...
            {/* 4. Calendar Legend */}
            <div className="mt-4 flex flex-wrap items-center justify-center gap-4 border-t border-[#f0e2cd]/80 pt-3 text-[11.5px] text-[#75695c]">
          ...
          {/* =========================================================
              PANEL 2: SELECTED DATE CEREMONY DETAILS (4 cols)
          ========================================================== */}
          <div className="flex flex-col justify-between rounded-[20px] border border-[#ebdcc4] bg-[#fffdfa] p-4 sm:p-5 shadow-2xs lg:col-span-4 h-full">
            <div>
              {/* Panel Top Heading */}
              <div className="flex items-center justify-between border-b border-[#ebdcc4] pb-3 mb-3.5">
                ...
                {formattedSelectedDate && (
                  <span className="text-[12px] font-bold text-[#2b241d]">
                    {formattedSelectedDate}
                  </span>
                )}
              </div>

              {/* Ceremony Event Previews */}
                ...
                {selectedDateEvents.length > 0 ? (
                  selectedDateEvents.map((event) => {
                    ...
                    return (
                      <div
                        key={event.id}
                        className="overflow-hidden rounded-[14px] border border-[#ebdcc4] bg-[#fffdfa] shadow-xs transition-all duration-200 hover:border-[#d4872b]"
                      >
                        {/* Ceremony Photo with Category & Status Overlay */}
                        <div className="relative h-36 sm:h-40 w-full overflow-hidden bg-[#241a12]">
                          <img
                            src={eventImg}
                            alt={event.name}
                          ...
                          <div className="absolute top-2.5 left-2.5 right-2.5 flex items-center justify-between gap-1.5 z-10">
                            <span className="rounded-full bg-black/70 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-[#f5eedf]">
                              {event.category || "Sacred Puja"}
                            </span>
                            <span
                              className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-bold shadow-xs ${statusConfig.bg}`}
                            >
                          ...
                          <div className="absolute bottom-2 left-2.5 z-10">
                            <div className="inline-flex items-center gap-1 text-[10.5px] font-medium text-[#f5eedf]">
                        ...
                        <div className="p-3.5 sm:p-4">
                          {/* Date & Occasion */}
                          <div className="flex flex-wrap items-center gap-1.5 text-[11.5px] font-medium text-[#75695c]">
                          ...
                          {/* Ceremony Title */}
                          <h4 className="mt-1 font-serif text-[16px] sm:text-[17px] font-bold leading-snug text-[#2b241d] transition-colors hover:text-[#c77722] line-clamp-2">
                            <Link to={`/puja/${event.slug}`}>{event.name}</Link>
                          </h4>

                          {/* Muhurat / Timing */}
                          {muhuratTime && (
                            <div className="mt-1.5 inline-flex items-center gap-1.5 text-[11.5px] text-[#8a7a68]">
                              <Clock size={11} className="text-[#c77722] shrink-0" />
                          ...
                          {/* Purpose */}
                          {event.purpose && (
                            <div className="mt-1.5 flex items-center gap-1.5 text-[12px] text-[#5c4e3f]">
                              <Sparkles size={11} className="text-[#d4872b] shrink-0" />
                              <span className="line-clamp-1">{event.purpose}</span>
                            </div>
                          )}

                          {/* Bottom Row: Starting Price + Compact View Puja CTA */}
                          <div className="mt-3 flex items-center justify-between border-t border-[#f0e2cd] pt-2.5">
                            <div>
                              <span className="block text-[9px] font-bold uppercase tracking-wider text-[#8a7a68]">
                                Starts From
                              </span>
                            ...
                            <Link
                              to={`/puja/${event.slug}`}
                              className="inline-flex items-center gap-1.5 rounded-[8px] bg-[#eab12c] px-3.5 py-1.5 text-[12px] font-bold text-[#1c1308] shadow-2xs transition-all duration-200 hover:bg-[#dda018] hover:shadow-xs focus:outline-none focus-visible:ring-2 focus-visible:ring-[#eab12c]"
                            >
                              <span>View Puja</span>
                              <ArrowRight size={12} />
                            </Link>
                  ...
                  <div className="flex flex-col items-center justify-center py-8 px-3 text-center">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full border border-[#ebdcc4] bg-[#fffaf0] text-[#b36c1e]">
                      <CalendarIcon size={16} />
                    </div>
                    <h4 className="mt-2.5 font-serif text-[15px] font-bold text-[#2b241d]">
                      No rituals scheduled on this date
                    </h4>
                    <p className="mt-1 text-[12px] text-[#75695c] max-w-[220px]">
          ...
          {/* =========================================================
              PANEL 3: CHOOSE PUJA BY PURPOSE (4 cols)
          ========================================================== */}
